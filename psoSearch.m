function [populationBestFitness,populationBestPosition] = psoSearch(fitnessFunctionHandler,speciesSize,dimentionLimits,params)
%dimentionLimits
...
for i = 1:intmax('int32')
    for j = 1:speciesSize
        individualsCurrentFitness(j) = fitnessFunctionHandler(individualsCurrentPosition(:,j));
    end
...
%     clf;
%     plot(0:0.01:20,fitnessFunctionHandler(0:0.01:20),'-',individualsCurrentPosition,individualsCurrentFitness,'ro');
%     axis([0,20,-40,40]);    
%     pause(0.1);
end
[populationBestFitness,maxIndex] = max(individualsBestFitness);
